#include <stdlib.h>

typedef struct reg *no;
struct reg {
           int info; 
           struct reg *prox;
};

typedef struct {
 no prim, ult;
 int qte;
 } Descritor;
 
//----------------------------------------------------   
// Cria uma lista simplesmente encadeada com descritor- LSECD
//----------------------------------------------------
void cria_LSECD (Descritor *lista) {
  (*lista).prim = (*lista).ult = NULL;
  (*lista).qte = 0;
} 
//----------------------------------------------------   
// Cria uma  uma LSECD com apenas um nó
//----------------------------------------------------
void cria_um_no_LSECD (Descritor *lista, int info) {
  no p = (no) malloc (sizeof(struct reg));
  p->info = info;
  p->prox = NULL;
  (*lista).prim = (*lista).ult = p;
  (*lista).qte = 1;
} 
//----------------------------------------------------
// Mostra os elementos da lista com descritor
//----------------------------------------------------
void mostra_LSECD (Descritor lista) {
  if (lista.qte == 0) {
    printf ("\nLista vazia");
    return;
  }
  no p = lista.prim;
  printf ("\nElementos da lista: ");
  do {
    printf ("%d ",p->info);
    p = p->prox;
  } while (p != NULL);
  printf ("\nTotal de elementos: %d\n",lista.qte);
}
//----------------------------------------------------
// Inclui um elemento no inicio da lista com descritor
//----------------------------------------------------
void insere_inicio_LSECD (Descritor *lista, int info) {
  no p;
  p = (no)malloc(sizeof(struct reg));
  p->info = info;
  p->prox = (*lista).prim;
  (*lista).prim = p;
  (*lista).qte++;
  if ((*lista).qte == 1)
   (*lista).ult = p;
}
//----------------------------------------------------
// Inclui um elemento no final da lista com descritor
//----------------------------------------------------
void insere_final_LSECD (Descritor *lista, int info) {
  no p, q;
  p = (no)malloc(sizeof(struct reg));
  p->info = info;
  p->prox = NULL;
  if ((*lista).qte == 0)
    (*lista).prim = p;
  else
    (*lista).ult->prox = p;
  (*lista).ult = p;
  (*lista).qte++;
}
//----------------------------------------------------
// Inclui um elemento na lista ordenada com descritor
//----------------------------------------------------
void insere_ordenada_LSECD (Descritor *lista, int info) {
  no p = (no)malloc(sizeof(struct reg));
  p->info = info;
  if ((*lista).qte == 0 || (*lista).prim->info>info) {
    p->prox = (*lista).prim;
    (*lista).prim = p;
    if ((*lista).qte == 0)
     (*lista).ult = p;
  }
  else {
    no q = (*lista).prim, r;
    do {
      r = q;
      q = q->prox;
    } while (q != NULL && q->info < info); 
    p->prox = q; 
    r->prox = p;      
    if (q == NULL)
      (*lista).ult = p;
  } 
  (*lista).qte++;
}
//----------------------------------------------------
// Remove o ultimo elemento da lista com descritor
//----------------------------------------------------
int remove_final_LSECD (Descritor *lista) {
  if ((*lista).qte == 0)
    return 0;
  no p, q;
  p = (*lista).prim;
  while (p->prox) {
    q = p;
    p = p->prox; 
  }
  (*lista).qte--;
  if ((*lista).qte == 0) 
    (*lista).prim = (*lista).ult = NULL;
  else {
    (*lista).ult = q;
    q->prox = NULL;
  }
  free(p);
  return 1;
}
//----------------------------------------------------
// Programa Principal
//----------------------------------------------------
int main () {
  int info;
  Descritor lista;    
  char resp;
  cria_LSECD (&lista);
  // Insere elementos na lista
  printf ("I N S E R C A O\n");
  do {
    printf ("\nDigite um numero inteiro: ");
    scanf ("%d",&info);
    insere_ordenada_LSECD (&lista,info);
    mostra_LSECD (lista);
    printf ("\n\nContinua (S/N)? ");   
    do {
      resp = toupper(getch());
    } while (resp!='N' && resp!='S');
  } while (resp!='N');
  // Remove ultimo elemento da lista
  mostra_LSECD (lista);
  remove_final_LSECD (&lista);
  printf ("\n\nRemocao do ultimo elemento da lista realizada!\n");
  mostra_LSECD (lista);
  getch();
}        
